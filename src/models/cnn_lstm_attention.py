...
class CNNLSTMAttention(nn.Module):
    ...
    Architecture:
        ...
        BiLSTM: LSTM(128, 128, 2, bidirectional=True, batch_first=True)
                output: (batch, 30, 256)

        Multi-head Self-Attention:
        attn = MultiheadAttention(embed_dim=256, num_heads=4, dropout=0.1, batch_first=True)
        attn_out = attn(query=lstm_out, key=lstm_out, value=lstm_out)[0]
                (batch, 30, 256)

        Global Pooling:
        pool = attn_out.mean(dim=1)  (batch, 256)

        Classifier:
        Linear(256, 64) → ReLU → Dropout(0.3) → Linear(64, 7)
        Output: (batch, 7)  logits for 7 classes

    Parameter count: ~1.5M-3M (largest of the 3 models).
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pooled = attn_out.mean(dim=1)  # (batch, 256)
