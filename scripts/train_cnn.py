...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CNNModel on 3W oil well data.")
    ...
    ckpt_path = f"{models_dir}/cnnmodel.pt"
    ...
    np.savez(
        os.path.join(metrics_dir, "cnnmodel_predictions.npz"),
        preds=test_preds,
        probs=probs,
        labels=y_true,
    )
