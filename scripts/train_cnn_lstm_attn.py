...
def main() -> None:
    ...
    history_path = os.path.join(metrics_dir, "cnnlstmattention_training_history.csv")
    pd.DataFrame(history).to_csv(history_path, index_label="epoch")
    curve_path = os.path.join(figures_dir, "cnnlstmattention_training_curves.png")
    plot_training_curves(history, "CNNLSTMAttention", save_path=curve_path)
    print(f"  Training history saved → {history_path}")
    print(f"  Training curves saved → {curve_path}")

    ckpt_path = f"{models_dir}/cnnlstmattention.pt"
    ...
    np.savez(
        os.path.join(metrics_dir, "cnnlstmattention_predictions.npz"),
        preds=test_preds,
        probs=probs,
        labels=y_true,
    )
