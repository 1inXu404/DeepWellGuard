...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/License-Pending-lightgrey.svg" alt="License">
</p>

本项目为**《基于 CNN-LSTM-Attention 的油井不良事件检测系统设计》**的代码实现。
...
- 🔭 **多尺度与双重注意力机制**：
  ...
  - **SE Block (通道注意力)**：动态评估所选温度、压力与阀门开度传感器的权重，自动静音无效背景噪声。
  - **Multi-Head Temporal Attention (时间注意力)**：在 120 个时间步中自动聚焦最具决定性的异常关键帧。
- ⚡ **工业级流式推理流**：采用降采样、PyArrow 极速读取、向量化预处理与 GPU 批推理技术，实现 Web 端秒级响应。
- 📊 **完整实验链路**：包含数据预处理、CNN/Uni-LSTM/Bi-LSTM 基线、CNN-LSTM-Attention 主模型、消融实验、混淆矩阵、ROC 曲线和指标汇总。

---

## 📈 实验结果 (Results)

| 模型架构 | Accuracy | Weighted F1 | Macro F1 |
| :--- | ---: | ---: | ---: |
| **CNN-LSTM-Attention** | **0.9360** | **0.9375** | **0.9297** |
| Bi-LSTM | 0.9318 | 0.9328 | 0.9259 |
| Uni-LSTM | 0.9273 | 0.9281 | 0.9197 |
| CNN | 0.9200 | 0.9190 | 0.9108 |
...
│   ├── preprocess.py           # 数据预处理与 train/val/test 缓存生成
...
│   ├── detailed_metrics.py     # 单模型详细报告、计数混淆矩阵与 ROC 曲线
...
### 1. 环境安装
```powershell
git clone https://github.com/1inXu404/DeepWellGuard.git
cd DeepWellGuard
...
## 📄 许可证 (License)
当前仓库尚未包含独立的 `LICENSE` 文件。正式公开发布或复用前，请先补充许可证文件并同步更新本节说明。
